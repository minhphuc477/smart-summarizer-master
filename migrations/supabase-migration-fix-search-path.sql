-- Migration: Fix search_path vulnerability in all functions
-- Date: 2025-11-06
-- Description: Add SET search_path = '' to all functions to prevent search_path attacks
...
-- 4. update_pdf_status
DROP FUNCTION IF EXISTS public.update_pdf_status(uuid, text, text) CASCADE;
CREATE OR REPLACE FUNCTION public.update_pdf_status(p_pdf_id uuid, p_status text, p_error_message text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
-- 6. add_owner_as_member (has trigger dependency - use CASCADE)
...
AS $$
BEGIN
  -- Notification logic here (simplified for migration)
  RETURN OLD;
END;
$$;

-- Recreate triggers for notify_note_deleted
...
-- Recreate triggers for notify_note_updated
...
-- Recreate triggers for notify_note_created
...
-- 10. trigger_auto_categorize_note
...
-- Recreate triggers for trigger_auto_categorize_note
...
-- 13. cleanup_old_webhook_deliveries
...
-- 15-19. Update triggers (CASCADE to handle trigger dependencies)
DROP FUNCTION IF EXISTS public.update_pdf_updated_at() CASCADE;
CREATE OR REPLACE FUNCTION public.update_pdf_updated_at()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
CREATE TRIGGER update_pdfs_updated_at
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.update_pdf_updated_at();
...
-- Recreate triggers for update_updated_at_column (used by multiple tables)
...
-- 18. search_pdfs
DROP FUNCTION IF EXISTS public.search_pdfs(text, uuid, int) CASCADE;
CREATE OR REPLACE FUNCTION public.search_pdfs(p_query text, p_user_id uuid DEFAULT auth.uid(), p_limit int DEFAULT 20)
RETURNS TABLE (
  id uuid,
  filename text,
  status text,
...
BEGIN
  ...
  WHERE p.user_id = p_user_id
    AND p.filename ILIKE '%' || p_query || '%'
  ORDER BY p.created_at DESC
  LIMIT p_limit;
END;
$$;

-- 19. create_auto_version_snapshot
...
-- 20. get_pdf_stats
DROP FUNCTION IF EXISTS public.get_pdf_stats(uuid) CASCADE;
CREATE OR REPLACE FUNCTION public.get_pdf_stats(p_user_id uuid DEFAULT auth.uid())
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
-- 23. match_notes (semantic search)
...
-- 29. create_mention_notifications
...
-- 33. increment_template_use_count
