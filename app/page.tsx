...
export default function Home() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }: { data: { session: Session | null } }) => {
      console.log('🔐 [Auth] Session loaded:', session ? `User: ${session.user.email}` : 'No session');
      setSession(session);
      setIsLoading(false);
    });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event: string, session: Session | null) => {
      console.log('🔐 [Auth] Auth state changed:', _event, session ? `User: ${session.user.email}` : 'No session');
      setSession(session);
      setIsLoading(false);
    });
